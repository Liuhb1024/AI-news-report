...
class ReportGenerator:
    ...
    def generate_full_report(self, enriched_data, date_str):
        ...
        for i, direction in enumerate(directions, 1):
            print(f"<br />🔍 分析方向 {i}/{len(directions)}: {direction['name']}...")
        ...
        # 组合完整报告
        full_report = "<br /><br />".join(report_parts)
...
"""
        ...
        for i, direction in enumerate(directions, 1):
            ...
            summary += f"{i}. **{direction['name']}** {'⭐' * min(int(confidence) if isinstance(confidence, (int, float)) else 0, 10)}<br />"
            summary += f"   - 置信度: {confidence}/10 | 紧迫性: {urgency} | 类别: {category} | 门槛: {threshold}<br /><br />"
...
"""
        ...
        if news_list:
            for i, news in enumerate(news_list[:5], 1):
                fallback += f"{i}. [{news.get('source', '未知')}] {news.get('title', '')}<br />"
        else:
            fallback += "暂无相关资讯<br />"
        
        fallback += "<br />---<br /><br />"
        fallback += "**注意**：由于AI分析服务暂时不可用，以上为基础信息汇总。建议结合资讯内容人工进一步研判。<br /><br />---<br />"
...
"""
    ...
    def save_report(self, report_content, date_str):
        ...
        print(f"<br />✅ 报告已保存到: {filename}")
        
        # 统计信息
        word_count = len(report_content)
        line_count = report_content.count('<br />')
    ...
    def _save_summary(self, report_content, date_str):
        ...
        try:
            ...
            summary_file = f"summary_{date_str}.txt"
            with open(summary_file, 'w', encoding='utf-8') as f:
                f.write(f"投资机会分析摘要 - {date_str}<br />")
                f.write("="*60 + "<br /><br />")
                f.write('<br />'.join(summary_lines))
                f.write(f"<br /><br />{'='*60}<br />")
                f.write(f"完整报告请查看: report_{date_str}.md<br />")
    ...
    def _export_pdf_simple(self, report_content, date_str):
        ...
        try:
            ...
            pdf_path = f"report_{date_str}.pdf"
    ...
    def _maybe_save_pdf(self, report_content, date_str):
        ...
        try:
            ...
            html_path = f"report_{date_str}.html"
...
def load_enriched_data(date_str):
    """加载补充后的数据"""
    filename = f"enriched_{date_str}.json"
